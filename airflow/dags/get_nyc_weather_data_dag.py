...
get_nyc_weather_data = GetDarkSkyWeatherOperator(
    ...
    latitude=config['DARKSKY']['DARKSKY_LATITUDE'],
    longitude=config['DARKSKY']['DARKSKY_LONGITUDE'],
    forecast_date_str='{ds}',
    csv_output_filepath='nyc_weather_{ds}.csv',
)
...
save_nyc_weather_data_to_S3 = SaveFileToS3Operator(
    ...
    dag=dag,
    provide_context=True,
    s3_conn_id='s3_nyc_weather',
    s3_bucket='nyc-weather-data-us-east-2',
    s3_key='{execution_date.year}/{execution_date.month}/nyc_weather_{ds}.csv',
    local_filepath='nyc_weather_{ds}.csv',
    replace=True
)
